...
warn contains iam.finding("info", sprintf("KMS 변경: `%s` (%s)", [rc.address, concat(", ", rc.change.actions)]), "키를 사용하는 서비스와 기존 암호문에 미치는 영향을 함께 확인해 주세요.") if {
	some rc in changes
}
...
deny contains iam.finding("high", sprintf("KMS 키 삭제 또는 교체: `%s`", [rc.address]), "삭제 예약 중에는 키를 사용할 수 없습니다. 삭제 완료 후에는 키와 해당 키가 필요한 암호문을 복구할 수 없으므로 데이터 보존 기간과 복구 계획을 확인해 주세요.") if {
	some rc in changes
	rc.type in key_types
	iam.acted(rc, "delete")
}

deny contains iam.finding("high", sprintf("KMS 키 비활성화: `%s`", [rc.address]), "이 키에 의존하는 암호화·복호화가 중단됩니다. 연결된 로그 수집과 데이터 접근에 미치는 영향을 확인해 주세요.") if {
	some rc in live
	rc.type in key_types
...
warn contains iam.finding("warn", sprintf("비활성 KMS 키 설정: `%s`", [rc.address]), "이 상태에서는 키를 사용할 수 없습니다. 외부 키 재료 import 대기 등 의도된 설정인지 확인해 주세요.") if {
	some rc in live
	rc.type in key_types
	some field in {"is_enabled", "enabled"}
	rc.change.after[field] == false
...
deny contains iam.finding("high", sprintf("KMS 정책 잠금 방지 검사 우회: `%s`", [rc.address]), "정책을 다시 변경할 권한을 잃을 수 있습니다. bypass_policy_lockout_safety_check 를 켜야 하는 사유와 복구 주체를 확인해 주세요.") if {
	some rc in live
...
warn contains iam.finding("warn", sprintf("KMS 자동 회전 비활성: `%s`", [rc.address]), "자동 회전을 지원하는 키입니다. 로그 보존과 키 관리 기준에 맞춰 회전을 켤지 확인해 주세요.") if {
	some rc in live
	rc.type == "aws_kms_key"
...
warn contains iam.finding("warn", sprintf("KMS 삭제 대기 기간 단축: `%s`", [rc.address]), "삭제를 취소할 수 있는 시간이 줄어듭니다. 복구 절차에 필요한 기간을 확보했는지 확인해 주세요.") if {
	some rc in live
	rc.type in key_types
	rc.change.after.deletion_window_in_days < rc.change.before.deletion_window_in_days
}

warn contains iam.finding("warn", sprintf("KMS 관리 설정 미확정: `%s`", [rc.address]), "활성화·자동 회전·정책 잠금 방지 설정 중 plan 시점에 확정되지 않은 값이 있습니다. 적용 전 최종 값을 확인해 주세요.") if {
	some rc in live
	rc.type in policy_types
...
warn contains iam.finding("warn", sprintf("KMS 키 정책 미확정 또는 해석 불가: `%s`", [rc.address]), "plan 에서 정책 본문을 확인할 수 없습니다. 기본 정책이나 별도 aws_kms_key_policy 를 포함해 실제 적용할 정책을 확인해 주세요.") if {
	some rc in live
	rc.type in policy_types
	not policy_doc(rc)
}
...
deny contains iam.finding("high", sprintf("조건 없는 전체 주체의 KMS 사용 허용: `%s`", [entry.rc.address]), "Principal 의 와일드카드가 모든 계정의 주체를 허용합니다. 계정·역할 범위를 제한해 주세요. 키 정책의 Resource 와일드카드는 이 검사 대상이 아닙니다.") if {
	some entry in statements
	wildcard_principal(entry.statement)
	not has_conditions(entry.statement)
}

warn contains iam.finding("warn", sprintf("조건부 전체 주체의 KMS 사용 허용: `%s`", [entry.rc.address]), "조건이 허용 계정·역할을 실제로 제한하는지 확인해 주세요. 조건이 있다는 사실만으로 안전하다고 판정하지 않습니다.") if {
	some entry in statements
	wildcard_principal(entry.statement)
