...
function gather(findingsDir, expectedDirs, skippedDirs, namespace) {
  ...
  const skipped = Array.isArray(skippedDirs)
    ? skippedDirs.filter((d) => !expected.includes(d)).sort()
    : [];
  const findings = [];
  const failed = [];
  let legs = 0;
  for (const dir of expected) {
    ...
    try {
      if (!fs.existsSync(file) || fs.existsSync(path.join(artifactDir, 'plan-failed')))
...
function unchangedPlatform(findingsDir, expectedDirs) {
  return (Array.isArray(expectedDirs) ? expectedDirs : [])
    .filter((dir) => {
      ...
      try {
        return (
          JSON.parse(fs.readFileSync(path.join(artifactDir, 'plan-status.json'), 'utf8'))
            .no_changes === true
        );
      } catch {
        return false;
...
function build(
  { findings, failed, expected, skipped, complete },
  { repoUrl, baseRef, sha, runUrl },
) {
  const footer =
    `<sub>[차단] 항목 외에는 병합을 막지 않습니다. ` +
    `[규칙](${repoUrl}/blob/${baseRef}/.github/policy/iam.rego)` +
    ` · ${sha.slice(0, 7)} · [로그](${runUrl})</sub>`;
  let unchecked;
  if (!expected.length && !skipped.length) {
    unchecked =
      '루트 모듈 목록을 얻지 못해 이 커밋의 IAM 변경을 검사하지 못했습니다 (discover 잡 로그 확인).';
  ...
  const lines = [
    ...
    ...findings
      .slice(0, MAX_ROWS)
      .map((f) => `| ${f.dir} | **${LABEL[f.level]}** | ${cell(f.text)} | ${cell(f.why)} |`),
  ];
  ...
  if (hasHigh) {
    try {
      ...
      await github.rest.issues.createLabel({
        ...
        name: 'iam:high-risk',
        color: 'd93f0b',
        description: '검토가 필요한 IAM 변경',
      });
    }
    await github.rest.issues.addLabels({
      ...context.repo,
      issue_number: pr.number,
      labels: ['iam:high-risk'],
    });
  } else if (complete) {
    // 전부 검사했고 깨끗할 때만 걷는다 — 검사 못 한 커밋은 그대로 둔다
    try {
