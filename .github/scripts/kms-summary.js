...
module.exports = async ({
  ...
  if (skipped.length && expected.length) {
    lines.push(
      `${skipped.map((dir) => `\`${dir}\``).join(', ')}: 변경 영향이 없어 plan 을 생략했습니다.`,
      '',
    );
  }
  if (findings.length) {
    lines.push(
      '| 루트 | 구분 | 변경 | 이유 |',
      '| --- | --- | --- | --- |',
      ...rows(findings.slice(0, MAX_ROWS)),
      '',
    );
  }
  ...
  lines.push(
    `<sub>자문 검사이며 병합을 차단하지 않습니다. [규칙](${repoUrl}/blob/${pr.base.ref}/.github/policy/kms.rego)` +
