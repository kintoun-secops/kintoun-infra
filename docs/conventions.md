...
- PR: 모든 루트를 `fmt`·`tflint` 하고 변경 영향이 있는 루트만 `validate`·`plan` 한다. 루트별 plan 코멘트, IAM·KMS 가드 코멘트, apply 순서(wave) 코멘트를 단다.
...
**흐름** — 브랜치 → PR (CI 가 모든 루트 모듈을 fmt·tflint 하고 변경 영향이 있는 루트를 validate·plan 해
